...
class TrainTransform:
    ...
    def __call__(self, image, targets, input_dim):
        ...
        if len(boxes_t) == 0:
            ...
            boxes_t = boxes_o
            labels_t = labels_o
            keypoints_t = keypoints_o

        labels_t = np.expand_dims(labels_t, 1)

        targets_t = np.hstack((labels_t, boxes_t, keypoints_t))
        padded_labels = np.zeros((self.max_labels, 17))
