...
from yolox.utils import adjust_box_anns, get_local_rank, adjust_keypoint_ann
...
class MosaicDetection(Dataset):
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            if len(mosaic_labels):
                ...
                out_img_flag = (mosaic_labels[:, 5::3] < 0) + (mosaic_labels[:, 5::3] > 2 * input_w) + \
                    (mosaic_labels[:, 6::3] < 0) + (mosaic_labels[:, 6::3] > 2 * input_h)
                mosaic_labels[:, 7::3][out_img_flag] = 0
    ...
    def mixup(self, origin_img, origin_labels, input_dim):
        ...
        cp_keypoint_origin_np = adjust_keypoint_ann(
        ...
        if FLIP:
            ...
            cp_keypoint_origin_np[:, 0::3] = (
                origin_w - cp_keypoint_origin_np[:, 0::3]
            )
            mirro_kp = np.ones(cp_keypoint_origin_np.shape)
            mirro_kp[:, 0:3] = cp_keypoint_origin_np[:, 3:6]
            mirro_kp[:, 3:6] = cp_keypoint_origin_np[:, 0:3]
        ...
        keypoint_labels = cp_keypoint_transformed_np
        labels = np.hstack((box_labels, cls_labels, keypoint_labels))
