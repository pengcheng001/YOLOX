...
class Exp(BaseExp):
    def __init__(self):
        ...
        self.filter_min_area = 150

        # --------------  training config --------------------- #
        self.warmup_epochs = 5
        self.max_epoch = 200
        self.warmup_lr = 0
        self.basic_lr_per_img = 0.01 / 1024.0
    ...
    def filter_bbox(self, input, target):
        ...
        if filter_on:
            ...
            for batch in range(batch_size):
                ...
                area_flag = (gt[:, 3]*gt[:, 4]) >= self.filter_min_area
                slected_gt = gt[area_flag]
                remain_len = slected_gt.shape[0]
                new_target[batch, :remain_len, :] = slected_gt
            return new_target
        else:
            return target
