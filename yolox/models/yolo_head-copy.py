...
class YOLOXHead(nn.Module):
        ...
        for k, (cls_conv, reg_conv, stride_this_level, x) in enumerate(
            ...
            if self.training:
                output = torch.cat([reg_output, obj_output, cls_output, \
                                    keypoint_reg_k1_output, keypoint_reg_k2_output, \
                                    keypoint_reg_k3_output, keypoint_reg_k4_output,
                                    keypoint_cls_k1_output, keypoint_cls_k2_output,
                                    keypoint_cls_k3_output, keypoint_cls_k4_output], 1)
                ...
                output = torch.cat(
                    [reg_output, obj_output.sigmoid(), cls_output.sigmoid(), 
                     keypoint_reg_k1_output, keypoint_reg_k2_output, \
                     keypoint_reg_k3_output, keypoint_reg_k4_output, \
                     keypoint_cls_k1_output.sigmoid(), keypoint_cls_k2_output.sigmoid(), \
                     keypoint_cls_k3_output.sigmoid(), keypoint_cls_k4_output.sigmoid()], 1
                )
    ...
    def get_output_and_grid(self, output, k, stride, dtype):
        ...
        keypoint_reg_col_start = (bbox_ch-1)
        keypoint_reg_col_end = (keypoint_reg_col_start+8)
        output[..., keypoint_reg_col_start:keypoint_reg_col_end] = (output[..., keypoint_reg_col_start:keypoint_reg_col_end] + grid.repeat(1,1,4)) * stride
        return output, grid
    ...
    def decode_outputs(self, outputs, dtype):
        ...
        logger.info("outputs[..., keypoint_reg_col_start:keypoint_reg_col_end] shape: {}".format(outputs[..., keypoint_reg_col_start:keypoint_reg_col_end].shape))
        logger.info("grid.repeat(1,1,4) shape: {}".format(grids.repeat(1,1,4).shape))
        outputs[..., keypoint_reg_col_start:keypoint_reg_col_end] = (outputs[..., keypoint_reg_col_start:keypoint_reg_col_end] + grids.repeat(1,1,4)) * stride
        return outputs
    ...
    def get_losses(
        ...
        # logger.info("total num anchors: {} ".format(outputs.shape))
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                keypoint_cls_target_k1 = F.one_hot(gt_keypoint_cls_per_image_k1[matched_gt_inds].to(torch.int64), 2).to(torch.float32)
                keypoint_cls_target_k2 = F.one_hot(gt_keypoint_cls_per_image_k2[matched_gt_inds].to(torch.int64), 2).to(torch.float32)
                keypoint_cls_target_k3 = F.one_hot(gt_keypoint_cls_per_image_k3[matched_gt_inds].to(torch.int64), 2).to(torch.float32)
                keypoint_cls_target_k4 = F.one_hot(gt_keypoint_cls_per_image_k4[matched_gt_inds].to(torch.int64), 2).to(torch.float32)
        ...
        loss_keypoint_cls_k1 = (
            self.bcewithlog_loss(keypoint_cls_pres_k1.view(-1, 2)[fg_masks], keypoint_cls_k1_targets)
        ).sum() / num_fg
        
        loss_keypoint_cls_k2 = (
            self.bcewithlog_loss(keypoint_cls_pres_k2.view(-1, 2)[fg_masks], keypoint_cls_k2_targets)
        ).sum() / num_fg
        
        loss_keypoint_cls_k3 = (
            self.bcewithlog_loss(keypoint_cls_pres_k3.view(-1, 2)[fg_masks], keypoint_cls_k3_targets)
        ).sum() / num_fg
        
        loss_keypoint_cls_k4 = (
            self.bcewithlog_loss(keypoint_cls_pres_k4.view(-1, 2)[fg_masks], keypoint_cls_k4_targets)
        ).sum() / num_fg
        ...
        # + loss_keypoint_cls_k1 + loss_keypoint_cls_k2 + \
        #        loss_keypoint_cls_k3 + loss_keypoint_cls_k4 + loss_peypoint_reg_k1 + loss_peypoint_reg_k1 + \
        #        loss_peypoint_reg_k2 + loss_peypoint_reg_k3 + loss_peypoint_reg_k4
