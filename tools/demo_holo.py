...
def make_parser():
    ...
    parser.add_argument("--bbox_min", default=None, type=int, help="filter bboox less than the min value")
    parser.add_argument("--bbox_max", default=None, type=int, help="filter bbbox more than the max value")
    parser.add_argument(
        "--fp16",
        dest="fp16",
        default=False,
...
class Predictor(object):
    ...
    def res_for_holo_banchmark(self, output, img_info, cls_conf, bbox_range=None):
        ...
        for i in range(len(bboxes)):
            ...
            w = x2 -x1 + 1
            h = y2 -y1 + 1
            if bbox_range is not None and (min(w,h) < bbox_range[0] or max(w,h) > bbox_range[1]):
                print("continue the box size out range: {}".format(bbox_range))
                continue
...
def image_demo(predictor, vis_folder, path, current_time, save_result, txt_folder, save_txt_res, bbox_range):
...
def main(exp, args):
    ...
    if args.tsize_h is not None and args.tsize_h is not None:
    ...
    if args.bbox_min is not None and args.bbox_max is not None:
        ...
        bbox_range = None
    predictor = Predictor(model, exp, HOLO_CLASSES, trt_file, decoder, args.device)
    current_time = time.localtime()
    if args.demo == "image":
        image_demo(predictor, vis_folder, args.path, current_time, args.save_result,txt_folder, args.save_txt_res, bbox_range)
